...
impl RenderedAlertContent {
    /// Tries constructing a new [RenderedAlertContent] from message content. Returns [Result::Err] if the resulting [RenderedAlertContent] would be too big.
    ///
    /// # Arguments
    ///
    ...
    /// Tries appending a [RenderedAlertContent]. Returns an [Result::Err] if the resulting [RenderedAlertContent] would be too large. In this case `self` remains unmodified.
    ///
    /// # Arguments
    ///
...
#[derive(Debug, Clone, PartialEq, Eq)]
/// [RenderedAlertContent] together with [RenderedAlertMetadata] to track throughput time and sending attempts.
