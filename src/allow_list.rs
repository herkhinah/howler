//! allow list containing user ids and server names
...
/// allow list storing matrix user ids and server names, we implement
/// Deserialize our selves to validate that only valid matrix user identifiers
/// and server names are used
#[derive(Debug, Clone)]
pub struct AllowList(HashSet<String>);

impl AllowList {
	/// check if user id is allowed (either the exact user id or the server name
	/// of the user id is in the allow list)
	pub fn contains(&self, user_id: &UserId) -> bool {
		self.0.contains(user_id.as_str()) || self.0.contains(user_id.server_name().as_str())
	}
}

/// empty struct that implements `serde::de::Visitor` for validation of user ids
/// and server names
		...
		A: serde::de::SeqAccess<'de>,
	{
		...
		while let Some(str) = seq.next_element::<String>()? {
				...
				.is_ok()
			{
				...
				return Err(serde::de::Error::invalid_type(serde::de::Unexpected::Seq, &self));
			}
		}
