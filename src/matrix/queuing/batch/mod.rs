...
/// The rendered alerts of a single room waiting for the batch duration timeout
/// starting from the oldest queued alert
#[derive(Debug)]
...
/// And there is [Self::waiting]. If a freshly rendered alert arrives and there
/// are no alerts for the target room ready to send they get inserted here.
/// The queue waits [Self::batch_interval] of time for other alerts for the same
/// target room to arrive and then moves them into [Self::ready].
///
/// If alerts that failed to send or failed to federate get requeued they get
/// directly inserted into [Self::ready] and the [Self::waiting] value for that
/// room get emptied without waiting for the batch duration timeout.
#[derive(Debug)]
pub struct BatchQueue {
	...
	/// RenderedAlertMetadata) of the oldest alert queued in [Self::waiting] for
	/// a specific room and moving those from [Self::waiting] to [Self::ready]
...
impl BatchQueue {
	...
	/// # Arguments
	///
	/// * `batch_interval` - the time interval the queue waits between the first
	///   alert arriving for a room to batch
	...
	/// queues new incoming alerts from the
	/// [crate::alert_renderer::AlertRenderer]
	///
	/// # Arguments
	///
	...
	pub fn queue(&mut self, room_id: OwnedRoomId, mut entry: RenderedAlert) {
		...
		match self.waiting.entry(room_id.clone()) {
			Entry::Occupied(mut occupied) => {
				let WaitingRoomEntries { timeout_queue_key: delay_queue_key, entries, oldest } =
					occupied.get_mut();

				if oldest > &mut entry.metadata.arrival {
					*oldest = entry.metadata.arrival;
					self.timeout_queue.reset_at(delay_queue_key, *oldest + self.batch_interval);
				}
			...
			Entry::Vacant(vacant) => {
				...
				let delay_queue_key =
					self.timeout_queue.insert_at(room_id, oldest + self.batch_interval);
	...
	/// get an iterator over the ready to send entries. the order of the rooms
	/// is random so we don't prefer certain rooms because of their hash value
...
/// This Stream is responsible for putting alerts from the batch queue into the
/// ready hashmap and notifies the caller that there are new alerts ready for
/// sending
	...
	fn poll_next(
		...
		match timeout_queue.poll_next(cx) {
			Poll::Ready(Some(expired)) => {
				...
				let WaitingRoomEntries { entries, .. } =
					self.waiting.remove(&room).expect("there must be entries queued");
