...
impl BotMetrics {
	...
	pub(crate) fn new(bot_id: &UserId) -> Result<Self, prometheus::Error> {
	...
	pub(crate) fn record_message_send(&self, room: &RoomId) {
		self.m_room_message_req.with_label_values(&[room.as_str()]).inc();
	}

	/// counts number of failed m.room.message event requests
	pub(crate) fn record_message_send_error(
		&self,
		room: &RoomId,
		error: Option<&matrix_sdk::ruma::api::client::Error>,
	) {
